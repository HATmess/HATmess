...
def svlan_profile ():
    ...
    for row in range (2 ,sheet.max_row ):
        ...
        if not port in port_list:
            ...
            if len(port_list) == 1:
                lldp.append(f"set interfaces {port} unit 4021 vlan-id 4021\n")
                lldp.append(f"set interfaces {port} unit 4021 family inet mtu 1500\n")
                lldp.append(f"set interfaces {port} unit 4021 family inet address {device['ospf_interco']}/30\n")
                lldp.append(f"set routing-instances RESIDENTIAL protocols ospf area 0.0.0.0 interface {port}.4021\n")
                lldp.append(f"set routing-instances RESIDENTIAL interface {port}.4021\n")
...
if __name__ == "__main__":
    ...
    layout = [  
            ...
            [sg.Listbox(values=lbox , size = (15, 5), key = '_FLOATING_LISTBOX_', enable_events = True) , sg.Checkbox('OSPF', default=False)],
